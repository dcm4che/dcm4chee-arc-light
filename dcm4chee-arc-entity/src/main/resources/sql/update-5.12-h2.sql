...
update series set sop_cuid = (select sop_cuid from instance join location on instance_fk = instance.pk
  where series_fk = series.pk and object_type = 0 limit 1 );
update series set tsuid = (select tsuid from instance join location on instance_fk = instance.pk
  where series_fk = series.pk and object_type = 0 limit 1 );

alter table queue_msg alter device_name set not null;
alter table series alter sop_cuid set not null;
alter table series alter tsuid set not null;
