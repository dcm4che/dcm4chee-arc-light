...
create table patient_id (pk bigint identity not null, pat_id varchar(255) not null, pat_id_type_code varchar(255), entity_id varchar(255), entity_uid varchar(255), entity_uid_type varchar(255), version bigint, patient_fk bigint not null, primary key (pk));
...
create unique nonclustered index UK_f1l196ykcnh7s2pwo6qnmltw7 on global_subscription (matchkeys_fk) where matchkeys_fk is not null;
...
create unique nonclustered index UK_pev4urgkk7id2h1ijhv8domjx on hl7psu_task (mpps_fk) where mpps_fk is not null;
...
create unique nonclustered index UK_1fuh251le2hid2byw90hd1mly on ian_task (mpps_fk) where mpps_fk is not null;
...
create unique nonclustered index UK_cqmmps9maltjybl44t4cck404 on instance_req (req_phys_name_fk) where req_phys_name_fk is not null;
...
create unique nonclustered index UK_44qwwvs50lgpog2cqmicxgt1f on mwl_item (perf_phys_name_fk) where perf_phys_name_fk is not null;
...
create unique nonclustered index UK_rj42ffdtimnrcwmqqlvj24gi2 on patient (pat_name_fk) where pat_name_fk is not null;
create unique nonclustered index UK_56r2g5ggptqgcvb3hl11adke2 on patient (resp_person_fk) where resp_person_fk is not null;
...
create unique nonclustered index UK_pu4p7k1o9hleuk9rmxvw2ybj6 on series (metadata_fk) where metadata_fk is not null;
create unique nonclustered index UK_5n4bxxb2xa7bvvq26ao7wihky on series (perf_phys_name_fk) where perf_phys_name_fk is not null;
...
create unique nonclustered index UK_bcn0jtvurqutw865pwp34pejb on series_req (req_phys_name_fk) where req_phys_name_fk is not null;
...
create unique nonclustered index UK_49eet5qgcsb32ktsqrf1mj3x2 on study (ref_phys_name_fk) where ref_phys_name_fk is not null;
...
create unique nonclustered index UK_kocdb2pxx2fymu1modhneb4xm on ups_req (req_phys_name_fk) where req_phys_name_fk is not null;
create index IDX5btv5autls384ulwues8lym4p on verify_observer (verify_datetime);
create unique nonclustered index UK_105wt9hglqsmtnoxgma9x18vj on verify_observer (observer_name_fk) where observer_name_fk is not null;
