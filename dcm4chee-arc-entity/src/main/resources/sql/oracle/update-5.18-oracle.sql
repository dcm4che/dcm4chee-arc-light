...
create table rejected_instance (
   ...
   series_iuid varchar2(255 char) not null,
   sop_cuid varchar2(255 char) not null,
   sop_iuid varchar2(255 char) not null,
   study_iuid varchar2(255 char) not null,
   reject_code_fk number(19,0),
   primary key (pk));
...
insert into rejected_instance (pk, created_time, sop_cuid, sop_iuid, reject_code_fk, series_iuid, study_iuid)
    (select rejected_instance_pk_seq.nextval, i.updated_time, i.sop_cuid, i.sop_iuid, i.reject_code_fk, se.series_iuid, st.study_iuid
