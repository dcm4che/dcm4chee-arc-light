...
create table metadata (pk numeric(18,0) not null, digest varchar(255), object_size numeric(18,0) not null, status integer not null, storage_id varchar(255) not null, storage_path varchar(255) not null, primary key (pk));
alter table series add metadata_update_time timestamp, add metadata_fk numeric(18,0), add inst_purge_time timestamp, add inst_purge_state integer;
