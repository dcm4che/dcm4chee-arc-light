create table mwl_item (pk bigint generated by default as identity, accession_no varchar(255) not null, created_time timestamp not null, modality varchar(255) not null, req_proc_id varchar(255) not null, sps_id varchar(255) not null, sps_start_date varchar(255) not null, sps_start_time varchar(255) not null, sps_status integer not null, study_iuid varchar(255) not null, updated_time timestamp not null, version bigint, dicomattrs_fk bigint not null, accno_issuer_fk bigint, patient_fk bigint not null, perf_phys_name_fk bigint, primary key (pk));
create table sps_station_aet (pk bigint generated by default as identity, station_aet varchar(255) not null, mwl_item_fk bigint not null, primary key (pk));
alter table mwl_item add constraint UK_6qj8tkh6ib9w2pjqwvqe23ko  unique (dicomattrs_fk);
create unique index UK_i6bbum2mvssf9l4aukvtbkjui on mwl_item (req_proc_id, sps_id);
