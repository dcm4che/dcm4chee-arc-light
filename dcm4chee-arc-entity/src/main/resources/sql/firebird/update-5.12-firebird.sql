...
update queue_msg set device_name = (select device_name from retrieve_task where queue_msg_fk = queue_msg.pk);
update queue_msg set device_name = (select device_name from export_task where queue_msg_fk = queue_msg.pk);
update queue_msg set device_name = 'dcm4chee-arc' where device_name is null;
...
update series set sop_cuid = (select sop_cuid from instance join location on instance_fk = instance.pk where series_fk = series.pk and object_type = 0 rows 1 );
update series set tsuid = (select tsuid from instance join location on instance_fk = instance.pk where series_fk = series.pk and object_type = 0 rows 1 );
