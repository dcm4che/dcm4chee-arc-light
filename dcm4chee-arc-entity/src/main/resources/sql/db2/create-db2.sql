...
create unique index UK_f1l196ykcnh7s2pwo6qnmltw7 on global_subscription (matchkeys_fk) exclude null keys;
...
create unique index UK_pev4urgkk7id2h1ijhv8domjx on hl7psu_task (mpps_fk) exclude null keys;
...
create unique index UK_1fuh251le2hid2byw90hd1mly on ian_task (mpps_fk) exclude null keys;
...
create unique index UK_cqmmps9maltjybl44t4cck404 on instance_req (req_phys_name_fk) exclude null keys;
...
create unique index UK_44qwwvs50lgpog2cqmicxgt1f on mwl_item (perf_phys_name_fk) exclude null keys;
...
create unique index UK_rj42ffdtimnrcwmqqlvj24gi2 on patient (pat_name_fk) exclude null keys;
create unique index UK_56r2g5ggptqgcvb3hl11adke2 on patient (resp_person_fk) exclude null keys;
...
create unique index UK_pu4p7k1o9hleuk9rmxvw2ybj6 on series (metadata_fk) exclude null keys;
create unique index UK_5n4bxxb2xa7bvvq26ao7wihky on series (perf_phys_name_fk) exclude null keys;
...
create unique index UK_bcn0jtvurqutw865pwp34pejb on series_req (req_phys_name_fk) exclude null keys;
...
create unique index UK_49eet5qgcsb32ktsqrf1mj3x2 on study (ref_phys_name_fk) exclude null keys;
...
create unique index UK_kocdb2pxx2fymu1modhneb4xm on ups_req (req_phys_name_fk) exclude null keys;
create index IDX5btv5autls384ulwues8lym4p on verify_observer (verify_datetime);
create unique index UK_105wt9hglqsmtnoxgma9x18vj on verify_observer (observer_name_fk) exclude null keys;
