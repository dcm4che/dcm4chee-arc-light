...
BEGIN { FS = "[ )]" }
{
    if ($1 == "create") {
        if ($3 == "code") {
                ...
                print "create table export_task (pk numeric(18,0) not null, batch_id varchar(255), created_time timestamp not null, device_name varchar(255) not null, exporter_id varchar(255) not null, modalities varchar(255), num_instances integer, scheduled_time timestamp not null, series_iuid varchar(64) not null, sop_iuid varchar(64) not null, study_iuid varchar(64) not null, updated_time timestamp not null, version numeric(18,0), queue_msg_fk numeric(18,0), primary key (pk));"
