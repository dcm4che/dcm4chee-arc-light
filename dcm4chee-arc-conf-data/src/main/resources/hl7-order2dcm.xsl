...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="PV1">
        ...
        <DicomAttribute tag="00380010" vr="LO">
            <Value number="1"><xsl:value-of select="string(field[19]/text())"/></Value>
        </DicomAttribute>
            ...
            <DicomAttribute tag="00380014" vr="SQ">
                <Item number="1">
                    ...
                    <DicomAttribute tag="00400031" vr="UT">
                        <Value number="1"><xsl:value-of select="$issuerOfAdmissionID" /></Value>
                    </DicomAttribute>
                </Item>
            </DicomAttribute>
        </xsl:if>
    ...
    <xsl:template name="pregnancyStatus">
        ...
        <xsl:if test="normalize-space($ambulantStatus)">
          <DicomAttribute tag="001021C0" vr="US">
              <Value number="1"><xsl:if test="$ambulantStatus = 'B6'">3</xsl:if></Value>
          </DicomAttribute>
        </xsl:if>
    </xsl:template>
