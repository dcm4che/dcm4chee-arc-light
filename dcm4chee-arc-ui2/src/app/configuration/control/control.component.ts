...
export class ControlComponent implements OnInit {
    ...
    iconTexts = {
        activations: {
            hoverText: (device) => {
                try {
                    return (
                       this.appService['dcm4cheeArcConfig']['deviceNameUrlMap'][device.dcmuiDeviceURL] === this.appService.archiveDeviceName
                    ) ?
                        $localize `:@@this_device_is_currently_active:This device is currently active` :
                        $localize `:@@activate_this_device:Activate this device`
                } catch (e) {
    ...
    initCheck(retries) {
        ...
        if (
            ...
            (_.hasIn(this.appService, 'global.notSecure') && this.appService.global.notSecure)) {
            this.init();
        } else {
            if (retries) {
                setTimeout(() => {
 ...
 reloaded successfully`);
    ...
    setAsDefaultDevice(object) {
        ...
        if (_.hasIn(this.appService, 'dcm4cheeArcConfig.deviceNameUrlMap') &&
            this.appService.dcm4cheeArcConfig.deviceNameUrlMap[object.dcmuiDeviceURL]) {
