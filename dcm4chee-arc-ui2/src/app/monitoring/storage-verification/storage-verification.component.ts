...
export class StorageVerificationComponent implements OnInit, OnDestroy {
    ...
    setTableSchemas() {
        this.tableConfigNormal = {
            table: j4care.calculateWidthOfTable(
                this.service.getTableSchema(this, this.action, {grouped: false, filterObject: this.filterObject})
            ),
        ...
        this.tableConfigGrouped = {
            table: j4care.calculateWidthOfTable(
                this.service.getTableSchema(this, this.action, {grouped: true, filterObject: this.filterObject})
            ),
            filter: this.filterObject,
            search: '',
            showAttributes: false,
    ...
    initSchema() {
        this.filterSchema = j4care.prepareFlatFilterObject(
            this.service.getFilterSchema( this.devices, this.localAET, $localize `:@@count_param:COUNT ${((this.count || this.count == 0) ? this.count : '')}:count:`),
            3);
