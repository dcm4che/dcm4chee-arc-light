...
export class HttpErrorHandler {
    ...
    public handleError(error){
        if ((error._body && error._body != '') || _.hasIn(error,"message") || _.hasIn(error,"error") || _.hasIn(error, "[00000902].Value[0]")) {
            try{
                if (_.hasIn(error, "error.ambiguous") || _.hasIn(error, "error.failures")) {
                    let errorDetail = '';
                    if (_.hasIn(error, "error.pids")) {
                        let successfulPIDs = _.get(error, "error.pids");
                        errorDetail += "pids:" + "<br>\n";
                        _.forEach(successfulPIDs, (pid, count) => {
                            errorDetail += "pid:" + pid.pid + "\n";
                        })
                    }
                    if (_.hasIn(error, "error.ambiguous")) {
                        let ambiguousPIDs = _.get(error, "error.ambiguous");
                        errorDetail += "ambiguous:" + "<br>\n";
                        _.forEach(ambiguousPIDs, (pid, count) => {
                            errorDetail += "pid:" + pid.pid + ", count:" + pid.count + "\n";
                        })
                    }
                    if (_.hasIn(error, "error.failures")) {
                        let failedPIDs = _.get(error, "error.failures");
                        errorDetail += "failures:" + "<br>\n";
                        _.forEach(failedPIDs, (pid, errorMessage) => {
                            errorDetail += "pid:" + pid.pid + ", errorMessage:" + pid.errorMessage + "\n";
                        })
                    }
                    this.mainservice.setMessage({
                        'title': $localize `:@@http-error-handler.error:Error ${(error.status || '')}`,
                        'text': $localize`:@@supplement_issuer_response:Supplement issuer results in \n`,
                        'status': 'error',
